...
class WPDataset(Dataset):
    def __init__(self, source_path, tokenizer, device=torch.device('cuda:0'), max_length=700, add_prefix=False, add_special_token=False, bart=False):
        self.source = []
        self.target = []
        self.tokenizer = tokenizer
        self.device = device
    ...
    def collate_fn(self, samples):
        ...
        if not self.bart:
            ...
            source = [s for s in source_initial]
            target = [t for t in target_initial]
            labels = [t for t in target_initial]
        ...
        if not self.bart:
            ...
            new_source = [s + [self.tokenizer.pad_token_id] * (source_max_t - len(s)) for s in source]
        ...
        if not self.bart:
            ...
            new_target = [s + [self.tokenizer.pad_token_id] * (target_max_t - len(s)) for s in target]
            new_labels = [s + [self.tokenizer.pad_token_id] * (target_max_t - len(s)) for s in labels]
