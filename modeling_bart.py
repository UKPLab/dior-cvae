...
class LMFLayer(nn.Module):
    def __init__(self, hidden_size, latent_size, latent_lmf_rank):
...
class BartVAEEncoder(BartPretrainedModel):
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.end_layer = config.end_layer if config.end_layer else len(self.layers) - 1
        assert 0 <= self.begin_layer <= self.end_layer <= len(self.layers) - 1
    ...
    def parallelize(self, device_map=None):
        ...
        # ln_f to last
        self.layernorm_embedding = self.layernorm_embedding.to(self.last_device)
    ...
    def forward(
        ...
        if condition_hidden_states is None:
            condition_hidden_states = tuple([None] * len(self.layers))
        else:
            assert self.is_cvae
        ...
        all_post_mu = ()
        all_post_sigma = ()
        loops = zip(self.layers, past_key_values, self.pooling, condition_hidden_states, self.recog_network,
                    self.prior_network, self.reccurnt_cell_weight_ih, self.reccurnt_cell_weight_hh)
        ...
        for idx, (encoder_layer, layer_past, pooling_layer, condition, recog_network, prior_network, weight_ih, weight_hh) in enumerate(loops):
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                if compute_kl and self.begin_layer <= idx <= self.end_layer:
                    # update prior latent
                    ...
                    post_latent_representation = recog_network(
                        torch.cat((representation, prior_latent_representaion), dim=-1))
...
class BartVAEDecoderWithLMHead(BartPretrainedModel):
    """
    Transformer decoder consisting of *config.decoder_layers* layers. Each layer is a :class:`BartDecoderLayer`
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.register_buffer("final_logits_bias", torch.zeros((1, embed_tokens.num_embeddings)))
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.begin_layer <= idx <= self.end_layer:
                latent = all_latent[idx - self.begin_layer]
            else:
                latent = None
            # Model parallel
            if self.model_parallel:
...
class BartDella(BartPretrainedModel):
    def __init__(self, config: BartConfig):
        super().__init__(config)
        padding_idx, vocab_size = config.pad_token_id, config.vocab_size
        self.shared = nn.Embedding(vocab_size, config.d_model, padding_idx)
        ...
        self.decoder = BartVAEDecoderWithLMHead(config, self.shared)
    ...
    def _resize_final_logits_bias(self, new_num_tokens: int) -> None:
        old_num_tokens = self.decoder.final_logits_bias.shape[-1]
        if new_num_tokens <= old_num_tokens:
            new_bias = self.decoder.final_logits_bias[:, :new_num_tokens]
        else:
            extra_bias = torch.zeros((1, new_num_tokens - old_num_tokens), device=self.decoder.final_logits_bias.device)
            new_bias = torch.cat([self.decoder.final_logits_bias, extra_bias], dim=1)
    ...
    def train_model(self, input_ids, labels=None, attention_mask=None, condition=None, condition_mask=None):
        if condition is not None:
    ...
    def forward(
        ...
        #     return_dict=return_dict,
        # )
        # lm_logits = decoder_outputs.logits + self.decoder.final_logits_bias
        lm_logits = decoder_outputs.logits
